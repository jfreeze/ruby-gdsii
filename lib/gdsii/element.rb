...
module Gdsii
  ...
  class Element < Group
    ...
    def Element.read(file, *args)
      ...
      case rec.type
      when GRT_BOUNDARY
		  Boundary.read_el(file, *args)
      when GRT_TEXT
		  Text.read_el(file, *args)
      when GRT_PATH
		  Path.read_el(file, *args)
      when GRT_SREF
		  SRef.read_el(file, *args)
      when GRT_AREF
		  ARef.read_el(file, *args)
      when GRT_BOX
		  Box.read_el(file, *args)
      when GRT_NODE
		  Node.read_el(file, *args)
      else
